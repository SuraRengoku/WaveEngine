    ...
    class NumberBox : Control {
        ...
        public override void OnApplyTemplate() {
            ...
            if(GetTemplateChild("PART_textBlock") is TextBlock textBlock) {
                textBlock.MouseLeftButtonDown += OnTextBlock_Mouse_LBD; // press the button
                textBlock.MouseLeftButtonUp += OnTextBlock_Mouse_LBU; // release the button -> set value manually / if moved -> finish
                textBlock.MouseMove += OnTextBlock_Mouse_Move; // hold the button and move -> drag to set value
            }
        }
        ...
        private void OnTextBlock_Mouse_LBU(object sender, MouseButtonEventArgs e) {
            if(_captured) {
                ...
                _captured = false;
                e.Handled = true;
                // release the button without moving, then show the textBox to allow set the value maually
