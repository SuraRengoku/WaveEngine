...
namespace WaveEditor.ContentToolsAPIStructs {
    ...
    class GeometryImportSettings {
        ...
        public void FromContentSettings(Content.Geometry geometry) {
            ...
            ReverseHandedness = ToByte(settings.ReserveHandedness);
    ...
    class SceneData : IDisposable {
        ...
        public void Dispose() {
            ...
            GC.SuppressFinalize(this); // do not call finilizer because Data is already released
        }

        // finilizer(called by Garbage Collector)
        ~SceneData() {
            Dispose();
        }
    }
    ...
    static class ContentToolsAPI {
        ...
        public static void CreatePrimitiveMesh(Content.Geometry geometry, PrimitiveInitInfo info) {
            ...
            try {
                ...
                Marshal.Copy(sceneData.Data, data, 0, sceneData.DataSize); // sceneData.Data -> data
