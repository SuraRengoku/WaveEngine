...
namespace WaveEditor.GameProject {
    ...
    class Project : ViewModelBase{
        ...
        private void SetCommands() {
            ...
            //    _ = Task.Run(async () => {
            //        try {
            //            await BuildGameCodeDll(x);
            //        } catch (Exception ex) {
            //            await Application.Current.Dispatcher.InvokeAsync(() => {
            //                Logger.Log(MessageType.Error, $"Build failed: {ex.Message}");
            //                Debug.WriteLine($"Build error: {ex}");
            //            });
            //        }
            //    });
            //}, x => {
            //    try {
            //        return !VisualStudio.IsDebugging() && VisualStudio.BuildDone;
            //    } catch(Exception ex) {
            //        Debug.WriteLine($"BuildCommand CanExecute error: {ex.Message}");
            //        return false;
            //    }
            //});

            DebugStartCommand = new RelayCommand<object>(async x => await RunGame(true), x => !VisualStudio.IsDebugging() && VisualStudio.BuildSucceeded);
            DebugStartWithoutDebuggingCommand = new RelayCommand<object>(async x => await RunGame(false), x => !VisualStudio.IsDebugging() && VisualStudio.BuildSucceeded);
