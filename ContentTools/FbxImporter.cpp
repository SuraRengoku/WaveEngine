...
// 3) the library paths int the following section point to the correct location
...
void fbx_context::load_fbx_file(const char* file) {
	...
	bool iptSuccess = importer->Initialize(file, -1, _fbx_manager->GetIOSettings());
	if (iptSuccess) {
		iptSuccess = importer->Import(_fbx_scene);
	}
...
bool fbx_context::get_mesh_data(FbxMesh* fbx_mesh, mesh& m) {
	...
	for (s32 i{0}; i < num_indices; ++i) {
		const u32 v_idx{ static_cast<u32>(indices[i]) };
	...
	if (fbx_mesh->GetMaterialIndices(&mtl_indices)) {
		...
		for (s32 i{0}; i < num_ploys; ++i) {

			const s32 mtl_index{ mtl_indices->GetAt(i) };
			assert(mtl_index >= 0);
			m.material_indices.emplace_back(static_cast<u32>(mtl_index));
			if (std::find(m.material_used.begin(), m.material_used.end(), static_cast<u32>(mtl_index)) == m.material_used.end()) {
				m.material_used.emplace_back(static_cast<u32>(mtl_index));
			}
	...
	for (s32 i{0}; i < uv_set_count; ++i) {
		...
		if (fbx_mesh->GetPolygonVertexUVs(uv_names.GetStringAt(i), uvs)) {
			...
			for (s32 j{0}; j < num_uvs; ++j) {
				m.uv_sets[i].emplace_back(static_cast<f32>(uvs[j][0]), static_cast<f32>(uvs[j][1]));
			}
		}
	}
...
ImportFbx(const char* file, scene_data* data) {
	...
	// NOTE: anything that involves using the FBX SDK should be single-threaded
	{
		...
		if (fbx_context.is_valid()) {
			...
			// TODO: send error message to editor
			return;
		}
	}
