...
struct formatInfo {
    enum rawDataType : u8 {
        OTHER,                                                                  // no data / channels are different from each other
        INTEGER,                                                                // data type -> integer
        FLOATINGPOINT,                                                          // data type -> float

        count
    };
    u8 componentCount;                                                          // channels
    u8 sizePerComponent;                                                        // size of each channel: 0 -> compressed / unaveraged / less than 1
    u8 sizePerPixel;                                                            // size of each pixel: 0 -> compressed
    u8 rawDataType;                                                             // raw data type
};

constexpr formatInfo formatInfo_v1_0[] ={
